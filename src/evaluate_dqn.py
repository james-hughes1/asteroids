...
avg_reward, best_reward, best_frames, most_steps_frames = evaluate_policy(env, model, input_shape, args.device, num_frames=num_frames, max_steps=2000, num_eval_episodes=args.episodes)

print(f"Best episode reward = {best_reward:.2f}, Average reward over {args.episodes} episodes = {avg_reward:.2f}")
save_gif(best_frames[::len(best_frames) // 200 + 1], "gifs/best_episode.gif")
print(f"Longest episode steps = {len(most_steps_frames)}")
save_gif(most_steps_frames[::(len(most_steps_frames) // 200 + 1)], "gifs/most_steps_episode.gif")
env.close()
